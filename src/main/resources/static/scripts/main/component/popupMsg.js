        ...
        var oPopup = new Popup({
            ...
            ok: function () {
                ...
                $.ajax({
                    ...
                    } else if (oResult.data.successMsg==-1) {
                        that.error('用户名不存在');
                    } else if(oResult.data.errMsg==0) {  //插入msg失败
                        that.error('系统异常稍后重试');
                    } else if(oResult.data.successMsg==-2) {
                        that.error('不能给自己发私信');
                    } else if(oResult.data.successMsg==1) {//插入成功跳转至站内信列表
                        window.location.href = '/wenda/msg/list/0/50' //假装分页
