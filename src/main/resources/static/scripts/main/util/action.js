(function (window, undefined) {
    ...
    function fUnFollowQuestion(oConf) {
        var that = this;
        that.post({
            url: '/wenda/unfollowers/question',
            data: {questionId: oConf.questionId},
            call: oConf.call,
            error: oConf.error,
            always: oConf.always
    ...
    function fPost(oConf) {
        ...
        $.ajax({
            ...
            var nCode = oResult.data.successMsg;
            if (oResult.data.successMsg === -1) {
                // 未登录
                //alert('未登录');
                window.location.href = '/wenda/user/toLogin?next=' + window.encodeURI(window.location.href);
                return;
            }
