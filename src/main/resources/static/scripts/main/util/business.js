        ...
        $(document).on('click', '.js-follow-user', function (oEvent) {
            ...
            Action[bFollow ? 'unFollowUser' : 'followUser']({
                ...
                call: function (oResult) {
                    ...
                    //修改粉丝数量
                    //fansNum.html(oResult.data.msg+'粉丝');
                },
        ...
        $(document).on('click', '.js-follow-question', function (oEvent) {
            ...
            Action[bFollow ? 'unFollowQuestion' : 'followQuestion']({
                ...
                call: function (oResult) {
                    ...
                    if (bFollow) {
                        // 移除用户
                        oListEl.find('.js-user-' + oResult.data.id).remove();
                    } else {
                        // 显示用户
                        oListEl.prepend('<a class="zm-item-link-avatar js-user-' + oResult.data.id + '" th:href="/user/profile/' + oResult.data.id + '" data-original_title="' + oResult.data.name + '"><img src="' + oResult.data.headUrl + '" class="zm-item-img-avatar"></a>');
                    }
                },
                error: function (oResult) {
                    alert('出现错误，请重试');
